...
1. 打开 Visual Studio
2. 文件 -> 打开 -> CMake，选择项目根目录的 `CMakeLists.txt`
3. 选择 Release/Debug 配置
4. 生成 -> 全部生成

### 方法三：命令行编译（MSVC）

```cmd
cl /EHsc /std:c++11 stand_lie_demo.cpp ws2_32.lib /Fe:stand_lie_demo.exe
```
...
| 功能 | Linux | Windows |
|------|-------|---------|
| Socket 库 | POSIX socket | Winsock2 |
| 头文件 | `<sys/socket.h>`, `<arpa/inet.h>` | `<winsock2.h>`, `<ws2tcpip.h>` |
| 初始化 | 无需 | `WSAStartup()` |
| 清理 | 无需 | `WSACleanup()` |
| 关闭 socket | `close()` | `closesocket()` |
| 休眠函数 | `sleep()`, `usleep()` | `Sleep()` (毫秒) |
| 链接库 | `-pthread` | `ws2_32.lib` |
