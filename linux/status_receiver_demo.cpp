 ...
 * @brief 四足机器人状态接收Demo - 接收机器人上报的状态数据
 *
 * 编译: g++ -std=c++11 -pthread status_receiver_demo.cpp -o status_receiver_demo
 * 运行: ./status_receiver_demo
 *
 * ============================================================================
 *                              网络配置说明
 * ============================================================================
...
void receiverThread(int sock) {
    ...
    while (running) {
        ssize_t recv_len = recvfrom(sock, buffer, RECV_BUFFER_SIZE, 0,
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  四足机器人状态接收Demo" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    if (bind(sock, (struct sockaddr*)&local_addr, sizeof(local_addr)) < 0) {
        std::cerr << "[ERROR] 绑定端口 " << LOCAL_PORT << " 失败" << std::endl;
        close(sock);
        return -1;
    }
